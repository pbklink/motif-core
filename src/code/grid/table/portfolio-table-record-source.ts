...
import { TableRecordSource, UserTableRecordSource } from './table-record-source';

export class PortfolioTableRecordSource extends UserTableRecordSource {
    ...
    constructor() {
        super(TableRecordSource.TypeId.Portfolio);
        this.setName(PortfolioTableRecordSource.baseName +
            (++PortfolioTableRecordSource._constructCount).toString(10));
    ...
    override loadFromJson(element: JsonElement) {
        ...
        const definitionElementArray = element.tryGetElementArray(PortfolioTableRecordSource.jsonTag_DefinitionKeys);
        ...
        if (definitionElementArray !== undefined && definitionElementArray.length > 0) {
            ...
            for (const definitionElement of definitionElementArray) {
                ...
                if (definition === undefined) {
                    ...
                    if (typeId !== PortfolioTableRecordSource.definitionTypeId) {
    ...
    override saveToJson(element: JsonElement) {
        ...
        element.setElementArray(PortfolioTableRecordSource.jsonTag_DefinitionKeys, keyElementArray);
    }
    ...
    override addArray(value: TableRecordDefinitionArray): TableRecordSource.AddArrayResult {
...
export namespace PortfolioTableRecordSource {
    export const baseName = 'LitIvemId';

    export function createFromRecordDefinitionList(list: TableRecordSource): PortfolioTableRecordSource {
        const result = new PortfolioTableRecordSource();
